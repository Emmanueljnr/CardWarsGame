            ...
            for (Player player : players) {
                ...
                for (ICard card : player.getInPlay().getCards()) {
                    ...
                    String attack = getPrompt(
                            ...
                            new String[]{"Yes", "yes", "y", "No", "no", "n"}); //CHECK FOR VALID INPUTS
                    if (attack.equals("Yes") || attack.equals("yes") || attack.equals("y")) {
                        ...
                        if (target.equals("1")) { 
                            attackStrategy.performAttack(player, otherPlayer, card);
                            ...
                            // Notify observers about health change
                            for (IPlayerObserver observer : otherPlayer.getObservers()) {
                                observer.onHealthChange(otherPlayer.getName(), otherPlayer.getHealth());
                            }
                        }
                        // Else if Target is "2" then we attack the beast...
                        else {                          
                            ICard targetCard = otherPlayer.getInPlay().getCard(Integer.parseInt(target)- 2);
                            attackStrategy.performAttack(player, otherPlayer, card, targetCard);
                        }
                    }
                }
